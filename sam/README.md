...
```bash
noscrum$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
noscrum$ sam local start-api
noscrum$ curl http://localhost:3000/
```
...
```bash
noscrum$ sam logs -n HelloWorldFunction --stack-name noscrum --tail
```
